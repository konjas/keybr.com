...
import { VialUSB } from "./vial.js";

export const kb = (function () {
  return {
    getKeyMap: async (kbinfo) => {
      kbinfo.layers = await VialUSB.send(VialUSB.CMD_VIA_GET_LAYER_COUNT, [], {
        uint8: true,
        index: 1,
      });
      ...
      let offset = 0;
      const chunksize = 28;
      const alldata = await VialUSB.getViaBuffer(
    ...
    getFeatures: async (kbinfo) => {
      ...
      const counts = await VialUSB.sendVial(
        VialUSB.CMD_VIAL_DYNAMIC_ENTRY_OP,
        [],
      );

      const macro_count = await VialUSB.send(
        VialUSB.CMD_VIA_MACRO_GET_COUNT,
        [],
        { index: 1 },
      );
      // size is a byte followed by a big-endian short, short is size.
      let macros_size = await VialUSB.send(
        VialUSB.CMD_VIA_MACRO_GET_BUFFER_SIZE,
        [],
        { unpack: "B>H", index: 1 },
      );
    ...
    getKeyLayout: async (kbinfo) => {
      ...
      kbinfo.keylayout = KLE.deserializeToKeylayout(
        kbinfo,
        kbinfo.payload.layouts.keymap,
      );
